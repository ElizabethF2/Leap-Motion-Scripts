...
def on_message(ws, message):
    ...
    pointerFinger = [maxint,maxint,maxint]
    for finger in frame["pointables"]:
        if finger["tipPosition"][2] < pointerFinger[2]:
            pointerFinger = finger["tipPosition"]
    ...
    if len(frame["pointables"]) == 1 and not(click):
        click = True
        mouse.toggle(click, mouse.LEFT_BUTTON)

    if len(frame["pointables"]) == 2 and click:
        click = False
        mouse.toggle(click, mouse.LEFT_BUTTON)
